...
parameters {
  ...
  real b_spell; // for name_H effect before and after onset
...
model {
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
         logy[s, b] ~ normal(alpha[1] + b_spell*spell_H[n] + RE[n], sigma); 
      }
      if(b > 1){
         logy[s, b] ~ normal(alpha[2] + b_spell*spell_H[n] + phi * logy[s, b-1] + RE[n], sigma); 
     }
    }
  }
}
...
generated quantities{
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
        log_lik[n] = normal_lpdf(logy[s, b] | alpha[1] + b_spell*spell_H[n] + RE[n], sigma); 
        y_tilde[n] = normal_rng(alpha[1] + b_spell*spell_H[n] + RE[n], sigma);
      }
      if(b > 1){
        log_lik[n] = normal_lpdf(logy[s, b] | alpha[2] + b_spell*spell_H[n] + phi * logy[s, b-1] + RE[n], sigma); 
        y_tilde[n] = normal_rng(alpha[2] + b_spell*spell_H[n] + phi * logy[s, b-1] + RE[n], sigma);
      }
    }
  }
}
