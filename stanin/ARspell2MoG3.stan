...
model {
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
        target += normal_lpdf( logy[s, b] | alpha[1] + b_spell[1]*spell_H[n] + RE[n], sigma_e); 
      }
      if(b > 1){
        lp_parts[1] = log_theta[1] + normal_lpdf( logy[s, b] | alpha[2] + (b_spell[2] + delta)*spell_H[n] + phi * logy[s, b-1] + RE[n], sigmap_e);
        lp_parts[2] = log_theta[2] + normal_lpdf( logy[s, b] | alpha[2] + b_spell[2]*spell_H[n] + phi * logy[s, b-1] + RE[n], sigma_e);      
...
generated quantities{
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
        log_lik[n] = normal_lpdf(logy[s, b] | alpha[1] + b_spell[1]*spell_H[n] + RE[n], sigma_e); 
        y_tilde[n] = normal_rng(alpha[1] + b_spell[1]*spell_H[n] + RE[n], sigma_e);
      }
      if(b > 1){
        log_lik[n] = log_sum_exp(
          log_theta[1] + normal_lpdf(logy[s, b] | alpha[2] + (b_spell[2] + delta)*spell_H[n] + phi * logy[s, b-1] + RE[n], sigmap_e),
          log_theta[2] + normal_lpdf(logy[s, b] | alpha[2] + b_spell[2]*spell_H[n] + phi * logy[s, b-1] + RE[n], sigma_e)); 
        theta_tilde = bernoulli_rng(theta); 
        if(theta_tilde) { 
          y_tilde[n] = normal_rng(alpha[2] + (b_spell[2]+delta)*spell_H[n] + phi * logy[s, b-1] + RE[n], sigmap_e);
        }
        else{
