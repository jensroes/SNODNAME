...
data {
  ...
  int<lower = 0, upper =5> name_H; // naming diversity
}
...
parameters {
  ...
  vector[2] beta; // for name_H effect before and after onset

  // hyper prior for naming effect
  real beta_mu;
...
transformed parameters{
  ...
  for(n in 1:N){
    RE[n] = u[subj[n]] + w[image[n]]; // + w[bigram[n]];
  } 
}
...
model {
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
        logy[s, b] ~ normal(alpha + beta[1]*name_H[n] + RE[n], sigma); 
      }
      if(b > 1){
        logy[s, b] ~ normal(alpha + beta[2]*name_H[n] + phi * logy[s, b-1] + RE[n], sigma); 
      }
    }
  }
}
...
generated quantities{
  ...
  int nKey = nKeys[s];
    for(b in 1:nKey){
      n += 1;
      if(b == 1){
        log_lik[n] = normal_lpdf(logy[s, b] | alpha + beta[1]*name_H[n] + RE[n], sigma); 
        y_tilde[n] = normal_rng(alpha + RE[n], sigma);
      }
      if(b > 1){
        log_lik[n] = normal_lpdf(logy[s, b] | alpha + phi * logy[s, b-1] + RE[n], sigma); 
        y_tilde[n] = normal_rng(alpha + beta[2]*name_H[n] + phi * logy[s, b-1] + RE[n], sigma);
      }
    }
  }
}
