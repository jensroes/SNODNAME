...
model {
  ...
  for(s in 1:nS){
    ...
    for(b in 1:nBS){ 
      n += 1;
      if(b == 1){
        lp_parts[1] = log_theta[1] + normal_lpdf(logy[s,b] | alpha + delta + RE[n], sigmap_e);
        lp_parts[2] = log_theta[2] + normal_lpdf(logy[s,b] | alpha + RE[n], sigma_e);
      }
      if(b > 1){
        lp_parts[1] = log_theta[1] + normal_lpdf(logy[s,b] | alpha + delta + phi*logy[s, b-1] + RE[n], sigmap_e);
    ...
    for(b in 1:nBS){
      ...
      if(b == 1){
        log_lik[n] = log_sum_exp(
 		      log_theta[1] + normal_lpdf(logy[s,b] | alpha + delta + RE[n], sigmap_e), 
   		    log_theta[2] + normal_lpdf(logy[s,b] | alpha + RE[n], sigma_e));
   		   theta_tilde = bernoulli_rng(theta[1]); 
         if(theta_tilde) { 
             y_tilde[n] = normal_rng(alpha + delta + RE[n], sigmap_e);
         }
         else{
      ...
      if(b > 1){      
	      log_lik[n] = log_sum_exp(
 		      log_theta[1] + normal_lpdf(logy[s,b] | alpha + delta + phi*logy[s, b-1] + RE[n], sigmap_e), 
   		    log_theta[2] + normal_lpdf(logy[s,b] | alpha + phi*logy[s, b-1] + RE[n], sigma_e));
   		  theta_tilde = bernoulli_rng(theta[1]); 
        if(theta_tilde) { 
          y_tilde[n] = normal_rng(alpha + delta + phi*logy[s, b-1] + RE[n], sigmap_e);
        }
        else{
