...
m <- readRDS(paste0(path, "ARspell2MoG3.rda" ))

#names(m)
param <- c("alpha", "b_spell", "delta", "phi", "theta", "sigmap_e", "sigma_e", "sigma_diff", "sigma") 
...
write_csv(msamps, "results/posteriorARspell2MoG3.csv")
...
samps <- rstan::extract(m,pars = param) %>% 
  as.data.frame() %>% as_tibble() %>%
  mutate(b_spell_delta.2 = b_spell.2 + delta,
         phi.2 = phi) %>%
